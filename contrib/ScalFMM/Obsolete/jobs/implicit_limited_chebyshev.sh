...
echo "Algorithm: implicit limited" >> $FINAL_DIR/stdout
...
mpiexec -n $NB_NODE $NUMACTL ./Build/Tests/Release/testBlockedImplicitChebyshev -nb $NB_PARTICLE -bs $GROUP_SIZE -h $TREE_HEIGHT -no-validation -$MODEL >> $FINAL_DIR/stdout 2>&1

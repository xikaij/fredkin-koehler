...
/// D(1/r)/Dx2, XKJ
template <class FReal>
struct FInterpMatrixKernelRx2 : FInterpAbstractMatrixKernel<FReal>
{
    static const KERNEL_FUNCTION_TYPE Type = HOMOGENEOUS;
    static const unsigned int NCMP = 1; //< number of components
    static const unsigned int NPV  = 1; //< dim of physical values
    ...
    FInterpMatrixKernelRx2() {}

    // copy ctor
    FInterpMatrixKernelRx2(const FInterpMatrixKernelRx2& /*other*/) {}

    static const char* getID() { return "ONE_OVER_R_CUBIC_DX"; }

    static void printInfo() { std::cout << "K(x,y)=(x1-y1)/r^3 with r=|x-y|" << std::endl; }
                        ...
                        const ValueClass& xs, const ValueClass& ys, const ValueClass& zs) const
    {
        ...
        if ( FMath::ConvertTo<double,ValueClass>(r2) < 1e-8 ) { return FMath::ConvertTo<ValueClass,double>(0.0); }
        else {
           const ValueClass one_over_r = FMath::One<ValueClass>() / FMath::Sqrt(r2);
           const ValueClass one_over_r3 = one_over_r*one_over_r*one_over_r;

           return one_over_r3 * diffx;
        }
    }
                                    ...
                                    ValueClass block[1], ValueClass blockDerivative[3]) const
    {
        ...
        if ( FMath::ConvertTo<double,ValueClass>(r2) < 1e-8 ) { block[0] = FMath::ConvertTo<ValueClass,double>(0.0); }
        else {
          const ValueClass one_over_r = FMath::One<ValueClass>() / FMath::Sqrt(r2);
          const ValueClass one_over_r3 = one_over_r*one_over_r*one_over_r;

          block[0] = one_over_r3 * diffx;
        }

        blockDerivative[0] = diffx; //Not being used, so an arbitrary value is assigned
        blockDerivative[1] = diffy;
        blockDerivative[2] = diffz;
    }
...
/// D(1/r)/Dy2, XKJ
template <class FReal>
struct FInterpMatrixKernelRy2 : FInterpAbstractMatrixKernel<FReal>
{
    static const KERNEL_FUNCTION_TYPE Type = HOMOGENEOUS;
    static const unsigned int NCMP = 1; //< number of components
    static const unsigned int NPV  = 1; //< dim of physical values
    ...
    FInterpMatrixKernelRy2() {}

    // copy ctor
    FInterpMatrixKernelRy2(const FInterpMatrixKernelRy2& /*other*/) {}

    static const char* getID() { return "ONE_OVER_R_CUBIC_DY"; }

    static void printInfo() { std::cout << "K(x,y)=(x2-y2)/r^3 with r=|x-y|" << std::endl; }
...
/// D(1/r)/Dz2, XKJ
template <class FReal>
struct FInterpMatrixKernelRz2 : FInterpAbstractMatrixKernel<FReal>
{
    static const KERNEL_FUNCTION_TYPE Type = HOMOGENEOUS;
    static const unsigned int NCMP = 1; //< number of components
    static const unsigned int NPV  = 1; //< dim of physical values
    ...
    FInterpMatrixKernelRz2() {}

    // copy ctor
    FInterpMatrixKernelRz2(const FInterpMatrixKernelRz2& /*other*/) {}

    static const char* getID() { return "ONE_OVER_R_CUBIC_DZ"; }

    static void printInfo() { std::cout << "K(x,y)=(x3-y3)/r^3 with r=|x-y|" << std::endl; }

...
#ifndef FP2PPARTICLECONTAINERWITHNORMAL_HPP
#define FP2PPARTICLECONTAINERWITHNORMAL_HPP

#include "Components/FBasicParticleContainer.hpp"

template<class FReal, int NRHS = 1, int NLHS = 1, int NVALS = 1>
class FP2PParticleContainerWithNormal : public FBasicParticleContainer<FReal, NVALS*(NRHS+4*NLHS) + 3, FReal> {
    typedef FBasicParticleContainer<FReal, NVALS*(NRHS+4*NLHS) + 3, FReal> Parent;
...
public:
    ...
    // To access the normals
    FReal* getNormalX(){
        return Parent::getAttribute(NVALS*(NRHS+4*NLHS));
    }

    FReal* getNormalY(){
        return Parent::getAttribute(NVALS*(NRHS+4*NLHS) + 1);
    }

    FReal* getNormalZ(){
        return Parent::getAttribute(NVALS*(NRHS+4*NLHS) + 2);
    }

    const FReal* getNormalX() const {
        return Parent::getAttribute(NVALS*(NRHS+4*NLHS));
    }

    const FReal* getNormalY() const {
        return Parent::getAttribute(NVALS*(NRHS+4*NLHS) + 1);
    }

    const FReal* getNormalZ() const {
        return Parent::getAttribute(NVALS*(NRHS+4*NLHS) + 2);
    }
...
#endif // FP2PPARTICLECONTAINERWITHNORMAL_HPP

...
template <>
InputParameters
validParams<MultiAppAddTransfer>()
{
...
void
MultiAppAddTransfer::initialSetup()
{
                                        ...
                                        MooseVariableFE & from_var)
{
  if (to_object->n_dofs(to_var.sys().number(), to_var.number()) >
         ...
         ++comp)
    {
      ...
      to_var.sys().solution().add(dof, from_value); // Changed 'set' to 'add' by XJ
    }
}
...
MultiAppAddTransfer::transfer(FEProblemBase & to_problem, FEProblemBase & from_problem)
{
  ...
  if ((to_mesh.n_nodes() != from_mesh.n_nodes()) || (to_mesh.n_elem() != from_mesh.n_elem()))
    mooseError("The meshes must be identical to utilize MultiAppAddTransfer.");
